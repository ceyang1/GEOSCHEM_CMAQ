...
REAL (kind=4), INTENT(IN)  :: IN_DATA( 2*NTHIK*( NCOLS + NROWS + 2*NTHIK ), NLAYS, NVARS)
REAL (kind=4), INTENT(OUT) :: OUT_DATA( 2*NTHIK*( NCOLS + NROWS + 2*NTHIK ), NLAYS, NVARS_BUFF)
...
! Open standard BCON file generated from CMAQ
...
if (ftype3d .eq. BNDARY3) then
  allocate (data( 2*nthik3d * (ncols3d + nrows3d + 2*nthik3d), nlays3d, standard_nvar), stat=stat)
  allocate (data_left( 2*nthik3d * (ncols3d + nrows3d + 2*nthik3d), nlays3d, left_nvar), stat=stat)
end if
